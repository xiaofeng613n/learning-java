package com.xf.basic.js;

/**
 * Created by xiaofeng on 2019/1/5
 * Description:
 */
import java.util.*;
...
public class JSEngine {
	...
	public static void callJsFunction() throws Exception{
		...
		engine.eval("function add (a, b, c) {return c; }");
		for (int i =0; i < 100000; i ++){
			Invocable jsInvoke = (Invocable) engine;
			Object res = jsInvoke.invokeFunction("add", new Object[] { 10, 5, "aaa" });
		}
	...
	public static void main(String[] args) throws Exception {
		...
		for (int i =0; i < 1000; i ++) {
			engineList.add(sem.getEngineByName("javascript"));
		}
...
//		callJsFunction();
		engine.eval("function add (a, b, c) {return c; }");
//		for (int i = 0; i < 100000; i ++) {
			engine.eval("var result = add (10 , 5, 1)");
